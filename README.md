...
**GitHub Pages:** deployed via GitHub Actions (`quarto render` → `_site/` → Pages)
...
│
│   [Website source files — Quarto]
├── _quarto.yml            ← Quarto project config (nav, theme, sidebar)
├── _footer.html           ← Custom footer (included via include-after-body)
├── .quartoignore          ← Excludes footer.md, rendercv/, scripts/, _future/
├── custom.css             ← Calvin identity styles
├── index.md
...
├── myst.yml               ← legacy, kept for reference
...
| `_quarto.yml`, `_footer.html`, `custom.css` | ✏️ Hand-edited |
